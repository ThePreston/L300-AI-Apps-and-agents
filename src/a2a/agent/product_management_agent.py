...
from agent_framework import (
    AgentThread,
    # ChatContext,  # Not used, removed
    # ChatAgent,  # OLD: Removed - replaced with Agent
    Agent,  # NEW: Correct class name in agent_framework
    BaseChatClient,
    tool,
)
...
class AgentFrameworkProductManagementAgent:
    ...
    # agent: ChatAgent  # OLD: ChatAgent doesn't exist
    agent: Agent  # NEW: Correct type annotation
    ...
    async def invoke(self, user_input: str, session_id: str) -> dict[str, Any]:
        ...
        # return self._get_agent_response(response.text)  # OLD: response.text may not exist
        # NEW: Extract text from response messages
    ...
    async def stream(
        ...
        # chunks: list[ChatContent] = []  # OLD: ChatContent doesn't exist
        chunks: list[str] = []  # NEW: Collect text chunks as strings

        # async for chunk in self.agent.run_stream(  # OLD: run_stream doesn't exist
        async for chunk in self.agent.run(  # NEW: Use run with stream=True
            messages=user_input,
            thread=self.thread,
            stream=True,  # NEW: Enable streaming
        ):
            # if chunk.text:  # OLD: Check for text attribute
            #     chunks.append(chunk.text)
            # NEW: Extract content from streaming updates
        ...
        if chunks:
            # yield self._get_agent_response(sum(chunks[1:], chunks[0]))  # OLD: sum() doesn't work with strings
            yield self._get_agent_response(''.join(chunks))  # NEW: Join string chunks

    def _get_agent_response(
        # self, message: ChatContent  # OLD: ChatContent doesn't exist
        self, message: str  # NEW: Message is a string
    ) -> dict[str, Any]:
        """Extracts the structured response from the agent's message content.

        Args:
            # message (ChatContent): The message content from the agent.  # OLD
            message (str): The message content from the agent.  # NEW
    ...
    async def _ensure_thread_exists(self, session_id: str) -> None:
        ...
        if self.thread is None or self.thread.service_thread_id != session_id:
            # self.thread = self.agent.get_new_thread(thread_id=session_id)  # OLD: parameter name changed
            self.thread = self.agent.get_new_thread(service_thread_id=session_id)  # NEW: Correct parameter name
